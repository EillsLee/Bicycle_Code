...
unsigned short _CRC_CHECK(unsigned char *Buf, unsigned char CRC_CNT)
{
...
void OutPut_Data(int x,int y,int z,int w)//主程序调用函数
{
  ...
  _CRC16 = _CRC_CHECK(databuf,8);
  databuf[8] = _CRC16&0xff;
  databuf[9] =(_CRC16&0xff00)>>8;
  
  for(i=0;i<10;i++)
    uart_putchar(UART_1,databuf[i]);//需要修改的串口字节发送函数！！！！！！！！！！！
}