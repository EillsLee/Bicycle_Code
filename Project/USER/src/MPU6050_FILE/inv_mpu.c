...
#define EMPL_NO_64BIT                   //STC16F40K不存在longlong类型

/* The following functions must be defined for this platform:
 * i2c_write(unsigned char slave_addr, unsigned char reg_addr,
 *      unsigned char length, unsigned char const *dat)
 * i2c_read(unsigned char slave_addr, unsigned char reg_addr,
 *      unsigned char length, unsigned char *dat)
...
/**
 *  @brief      Enable/disable dat ready interrupt.
 *  If the DMP is on, the DMP interrupt is enabled. Otherwise, the dat ready
 *  interrupt is used.
...
/**
 *  @brief      Read raw gyro dat directly from the registers.
 *  @param[out] dat        Raw dat in hardware units.
...
/**
 *  @brief      Read raw accel dat directly from the registers.
 *  @param[out] dat        Raw dat in hardware units.
 *  @param[out] timestamp   Timestamp in milliseconds. Null if not needed.
 *  @return     0 if successful.
 */
...
/**
 *  @brief      Read temperature dat directly from the registers.
 *  @param[out] dat        Data in q16 format.
 *  @param[out] timestamp   Timestamp in milliseconds. Null if not needed.
 *  @return     0 if successful.
 */
...
int mpu_configure_fifo(unsigned char sensors)
{
    ...
    /* Compass dat isn't going into the FIFO. Stop trying. */
 ...
 *  If @e sensors does not contain a particular sensor, disregard the dat
 ...
 *  @param[out] gyro        Gyro dat in hardware units.
 *  @param[out] accel       Accel dat in hardware units.
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    unsigned char dat[MAX_PACKET_LENGTH];
 ...
 *  @param[in]  dat        Bytes to write to memory.
 *  @return     0 if successful.
 */
 ...
 *  @param[out] dat        Bytes read from memory.
 *  @return     0 if successful.
 */
 ...
 *  sample. The hardware monitors the accel dat and detects any large change
 ...
 *  @param[in]  time        Duration in milliseconds that the accel dat must
...
uint8 run_self_test(void)
{
	...
	if (result == 0x3) 
	{
		/* Test passed. We can trust the gyro dat here, so let's push it down
		* to the DMP.
		*/
		float sens;
...
uint8 mpu_dmp_init(void)
{
	...
	{	 
		...
		if(res)return 2; 
		res=mpu_set_sample_rate(1000);	//设置采样率99
		if(res)return 3; 
...
int32 X, Y, Z;
uint8 mpu_dmp_get_dat(float *pitch,float *roll,float *yaw)
{
	...
	/* Gyro and accel dat are written to the FIFO by the DMP in chip frame and hardware units.
